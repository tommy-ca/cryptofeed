    ...
    def __init__(
        ...
        **kwargs,
        ...
        if subscription is not None:
            for channel in subscription:
                ...
                self.subscription[chan].update(
                    [self.std_symbol_to_exchange_symbol(symbol) for symbol in subscription[channel]]
                )
        ...
        if symbols and channels:
            ...
            self.subscription = dict.fromkeys(channels, symbols)
            ...
            for channel in subscription:
                for pair in subscription[channel]:
                    ...
                    if sum(map(len, sub.values())) == limit:
                        ret.append(
                            (
                                WSAsyncConn(addr, self.id, authentication=auth, subscription=sub, **options),
                                self.subscribe,
            ...
            if limit and count > limit:
                ...
                for add in addr:
                    ret.append(
                        (
                            WSAsyncConn(
                                add, self.id, authentication=auth, subscription=filtered_sub, **endpoint.options
                            ),
                            self.subscribe,
                            self.message_handler,
                            self.authenticate,
                ...
                ret.append(
                    (
                        WSAsyncConn(addr, self.id, authentication=auth, subscription=filtered_sub, **endpoint.options),
                        self.subscribe,
                        self.message_handler,
                        self.authenticate,
                    )
