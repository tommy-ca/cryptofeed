...
def _symbol_from_inst_id(common, inst_id: str, segment: int):
    base, quote = util.split_hyphen_symbol(inst_id)
    sym = common.Symbol()
    sym.base = base
    sym.quote = quote
...
def to_common_trade(native, md, cmn=None):
    ...
    out.price.CopyFrom(util.make_decimal(cmn, getattr(getattr(native, 'price', None), 'value', getattr(native, 'price', None) or getattr(native, 'px', None))))
    out.amount.CopyFrom(util.make_decimal(cmn, getattr(getattr(native, 'size', None), 'value', getattr(native, 'size', None) or getattr(native, 'sz', None))))
...
def to_common_l2_from_orderbook(native, md, cmn=None):
    ...
    for lvl in getattr(native, 'asks', []):
        ...
        pl.size.CopyFrom(util.make_decimal(cmn, getattr(getattr(lvl, 'size', None), 'value', getattr(lvl, 'size', None))))
        out.asks.append(pl)
    if getattr(native, 'seq_id', None):
        out.sequence_number = native.seq_id
    if getattr(native, 'event_timestamp', None):
        out.timestamp.CopyFrom(native.event_timestamp)
    if getattr(native, 'raw_data', None):
        out.raw_data = native.raw_data
