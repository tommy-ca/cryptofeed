...
def _symbol_from_inst_id(common, inst_id: str, segment: int):
    base, quote = util.split_base_quote_concat(inst_id)
    sym = common.Symbol()
    sym.base = base
    sym.quote = quote
    sym.symbol = f"{base}-{quote}" if quote else inst_id
...
def to_common_trade(native, md, cmn=None):
    ...
    out.symbol.CopyFrom(_symbol_from_inst_id(common, native.inst_id, native.segment))
    ...
    out.amount.CopyFrom(util.make_decimal(cmn, getattr(getattr(native, 'size', None), 'value', getattr(native, 'size', None))))
...
def to_common_funding(native, md, cmn=None):
    ...
    out = md.Funding()
    out.exchange = _exchange(common)
    # Map inst_id to Symbol using concat split (bitget often uses BTCUSDT)
    base, quote = util.split_base_quote_concat(getattr(native, 'inst_id', ''))
    sym = common.Symbol(); sym.base=base; sym.quote=quote; sym.symbol=f"{base}-{quote}" if quote else getattr(native,'inst_id','')
    sym.type = _instrument_type_from_segment(common, native.segment)
    out.symbol.CopyFrom(sym)
