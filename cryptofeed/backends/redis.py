            ...
            async with self.read_queue() as updates:
                async with conn.pipeline(transaction=False) as pipe:
                    for update in updates:
                        ...
                        pipe = pipe.zadd(
                            f"{self.key}-{record['metadata']['exchange']}-{record['metadata']['symbol']}"
                            if record.get("format") == "protobuf"
                            else f"{self.key}-{update['exchange']}-{update['symbol']}",
                            {
                                json.dumps(record): (
                                    record["metadata"]["receipt_timestamp"]
                                    if record.get("format") == "protobuf"
                                    else update[self.score_key]
                                )
                            },
                            nx=True,
                        )
                ...
                async with conn.pipeline(transaction=False) as pipe:
                    for update in updates:
                        if (
                            ...
                            and update.get("format") == "protobuf"
                        ):
