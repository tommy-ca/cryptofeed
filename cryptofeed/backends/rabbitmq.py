...
class RabbitCallback:
        ...
        if not self.conn:
            if self.exchange_mode:
                ...
                self.conn = await self.conn.declare_exchange(
                    self.exchange_name, self.exchange_type, durable=True, auto_delete=False
                )
            else:
                connection = await aio_pika.connect_robust(f"amqp://{self.host}", loop=asyncio.get_running_loop())
                self.conn = await connection.channel()
    ...
    async def write(self, data: dict):
        ...
        if self.exchange_mode:
            await self.conn.publish(aio_pika.Message(body=json.dumps(data).encode()), routing_key=self.routing_key)
        else:
            await self.conn.default_exchange.publish(
                aio_pika.Message(body=json.dumps(data).encode()), routing_key=self.routing_key
            )
