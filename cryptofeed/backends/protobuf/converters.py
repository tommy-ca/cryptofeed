"""Protobuf converter functions for market, order, and account data types."""
...
def funding_to_proto(funding_obj) -> funding_pb2.Funding:
    ...
    if (
        hasattr(funding_obj, "predicted_rate")
        and funding_obj.predicted_rate is not None
    ):
        proto.predicted_rate = str(funding_obj.predicted_rate)
    if (
        hasattr(funding_obj, "next_funding_time")
        and funding_obj.next_funding_time is not None
    ):
...
def orderbook_to_proto(orderbook_obj) -> order_book_pb2.Level2Book:
    ...
    if (
        hasattr(orderbook_obj, "sequence_number")
        and orderbook_obj.sequence_number is not None
    ):
...
def position_to_proto(position_obj) -> position_pb2.Position:
    ...
    if (
        hasattr(position_obj, "unrealised_pnl")
        and position_obj.unrealised_pnl is not None
    ):
