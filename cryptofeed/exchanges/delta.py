...
class Delta(Feed):
    id = DELTA
    websocket_endpoints = [
        WebsocketEndpoint("wss://socket.delta.exchange", sandbox="wss://testnet-socket.delta.exchange")
    ]
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        await self.book_callback(
            L2_BOOK, self._l2_book[symbol], timestamp, timestamp=self.timestamp_normalize(msg["timestamp"]), raw=msg
        )
                        ...
                        "channels": [
                            {
                                "name": c
                                if c != "candlestick_"
                                else c + self.candle_interval_map[self.candle_interval],
