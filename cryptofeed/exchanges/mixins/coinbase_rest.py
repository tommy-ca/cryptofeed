...
from cryptofeed.types import Balance, Candle, OrderBook, OrderInfo, Ticker, Trade
...
class CoinbaseRestMixin(RestExchange):
    ...
    async def _request(self, method: str, endpoint: str, auth: bool = False, body=None, retry_count=1, retry_delay=60):
        ...
        if method == "GET":
            data = await self.http_conn.read(
                f"{api}{endpoint}", header=header, retry_count=retry_count, retry_delay=retry_delay
            )
        elif method == "POST":
    ...
    async def _date_to_trade(self, symbol: str, timestamp: float) -> int:
        ...
        while True:
            ...
            elif timestamp > data[0]["time"].timestamp():
                lower = bound
                bound = (upper + lower) // 2
            else:
                upper = bound
    ...
    async def trades(self, symbol: str, start=None, end=None, retry_count=1, retry_delay=60):
        ...
        if start:
            ...
            data = await self._request(
                "GET", f"/products/{symbol}/trades", retry_count=retry_count, retry_delay=retry_delay
            )
            yield [self._trade_normalize(symbol, d) for d in data]

    async def ticker(self, symbol: str, retry_count=1, retry_delay=60):
        data = await self._request(
            "GET", f"/products/{symbol}/ticker", retry_count=retry_count, retry_delay=retry_delay
        )
        return Ticker(self.id, symbol, Decimal(data["bid"]), Decimal(data["ask"]), data["time"].timestamp(), raw=data)

    async def l2_book(self, symbol: str, retry_count=1, retry_delay=60):
        data = await self._request(
            "GET", f"/products/{symbol}/book?level=2", retry_count=retry_count, retry_delay=retry_delay
        )
    ...
    async def l3_book(self, symbol: str, retry_count=1, retry_delay=60):
        data = await self._request(
            "GET", f"/products/{symbol}/book?level=3", retry_count=retry_count, retry_delay=retry_delay
        )
        ret = OrderBook(self.id, symbol)
    ...
    async def place_order(
        self, symbol: str, side: str, order_type: str, amount: Decimal, price=None, client_order_id=None, options=None
    ):
        ...
        body = {"product_id": symbol, "side": "buy" if BUY else SELL, "size": str(amount), "type": ot}
    ...
    async def candles(
        ...
        assert interval in list(valid_intervals.keys()), (
