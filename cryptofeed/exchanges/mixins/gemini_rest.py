...
class GeminiRestMixin(RestExchange):
    ...
    rest_channels = (TRADES, TICKER, L2_BOOK, ORDER_STATUS, CANCEL_ORDER, PLACE_ORDER, BALANCES, TRADE_HISTORY)
    order_options = {
    ...
    async def ticker(self, symbol: str, retry_count=1, retry_delay=60):
        ...
        return {"symbol": symbol, "feed": self.id, "bid": Decimal(data["bid"]), "ask": Decimal(data["ask"])}
    ...
    async def balances(self):
        ...
        return {
            entry["currency"]: {"total": Decimal(entry["amount"]), "available": Decimal(entry["available"])}
            for entry in data
        }
