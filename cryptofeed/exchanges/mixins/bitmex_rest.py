...
from cryptofeed.defines import ASK, BID, BUY, L2_BOOK, SELL, TICKER, TRADES
...
class BitmexRestMixin(RestExchange):
    ...
    def _generate_signature(self, verb: str, url: str, data="") -> dict:
        ...
        signature = hmac.new(
            bytes(self.key_secret, "utf8"), bytes(message, "utf8"), digestmod=hashlib.sha256
        ).hexdigest()
        return {"api-expires": str(expires), "api-key": self.key_id, "api-signature": signature}
    ...
    async def _get(self, endpoint, symbol, retry_count, retry_delay):
        ...
        data = await self.http_conn.read(
            f"{self.api}{endpoint}", header=header, retry_count=retry_count, retry_delay=retry_delay
        )
