...
class BinanceRestMixin(RestExchange):
    ...
    rest_channels = (TRADES, ORDER_STATUS, CANCEL_ORDER, PLACE_ORDER, BALANCES, ORDERS, CANDLES)
    order_options = {
        LIMIT: "LIMIT",
        MARKET: "MARKET",
        FILL_OR_KILL: "FOK",
    ...
    async def orders(self, symbol: str = None):
        data = await self._request(
            ...
            "openOrders",
            auth=True,
            payload={"symbol": self.std_symbol_to_exchange_symbol(symbol)} if symbol else {},
        )
        return data
...
class BinanceFuturesRestMixin(BinanceRestMixin):
    ...
    rest_channels = (TRADES, ORDER_STATUS, CANCEL_ORDER, PLACE_ORDER, BALANCES, ORDERS, POSITIONS)

    async def place_order(
        self, symbol: str, side: str, order_type: str, amount: Decimal, price=None, time_in_force=None
    ):
        data = await super().place_order(
            symbol, side, order_type, amount, price=price, time_in_force=time_in_force, test=False
        )
        return data
