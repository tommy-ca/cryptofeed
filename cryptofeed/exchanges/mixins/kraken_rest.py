...
class KrakenRestMixin(RestExchange):
    ...
    rest_channels = (TRADES, TICKER, L2_BOOK, ORDER_STATUS, CANCEL_ORDER, PLACE_ORDER, BALANCES, ORDERS, TRADE_HISTORY)
    ...
    async def ticker(self, symbol: str, retry_count=1, retry_delay=60):
        ...
        data = await self._post_public(
            "/public/Ticker", payload={"pair": sym}, retry_count=retry_count, retry_delay=retry_delay
        )
    ...
    async def l2_book(self, symbol: str, retry_count=1, retry_delay=60):
        ...
        sym = self.std_symbol_to_exchange_symbol(symbol).replace("/", "")
        data = await self._post_public(
            "/public/Depth", {"pair": sym, "count": 200}, retry_count=retry_count, retry_delay=retry_delay
        )
    ...
    async def balances(self):
        ...
        return {
            cur_map.get(currency, currency): {"available": Decimal(value), "total": Decimal(value)}
