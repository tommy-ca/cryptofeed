...
from cryptofeed.defines import ASK, BID, BITSTAMP, BUY, L2_BOOK, L3_BOOK, SELL, TRADES
...
class Bitstamp(Feed, BitstampRestMixin):
    ...
    rest_endpoints = [
        RestEndpoint(
            "https://www.bitstamp.net", routes=Routes("/api/v2/trading-pairs-info/", l2book="/api/v2/order_book/{}")
        )
    ]
    websocket_channels = {
    ...
    async def _process_l2_book(self, msg: dict, timestamp: float):
        ...
        await self.book_callback(
            L2_BOOK, self._l2_book[pair], timestamp, timestamp=self.timestamp_normalize(ts), delta=delta, raw=msg
        )
    ...
    async def _process_l3_book(self, msg: dict, timestamp: float):
        ...
        await self.book_callback(
            L3_BOOK, self._l3_book[pair], timestamp, timestamp=self.timestamp_normalize(ts), raw=msg
        )
    ...
    async def message_handler(self, msg: str, conn, timestamp: float):
        ...
        if "bts" in msg["event"]:
            if msg["event"] == "bts:connection_established" or msg["event"] == "bts:subscription_succeeded":
                pass
            else:
    ...
    async def _snapshot(self, pairs: list, conn: AsyncConnection):
        ...
        for r, pair in zip(results, pairs):
            ...
            await self.book_callback(
                L2_BOOK, self._l2_book[std_pair], time.time(), timestamp=float(r["timestamp"]), raw=r
            )
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan in self.subscription:
            for pair in self.subscription[chan]:
                await conn.write(json.dumps({"event": "bts:subscribe", "data": {"channel": f"{chan}_{pair}"}}))
