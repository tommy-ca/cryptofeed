...
class CryptoDotCom(Feed):
    ...
    websocket_channels = {L2_BOOK: "book", TRADES: "trade", TICKER: "ticker", CANDLES: "candlestick"}
    request_limit = 100
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        for entry in msg["data"]:
            ...
            await self.book_callback(
                L2_BOOK, self._l2_book[pair], timestamp, timestamp=self.timestamp_normalize(entry["t"]), raw=entry
            )
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan, symbols in self.subscription.items():
            ...
            await conn.write(
                json.dumps({"method": "subscribe", "params": {"channels": [sym(chan, symbol) for symbol in symbols]}})
            )
            await asyncio.sleep(1)
