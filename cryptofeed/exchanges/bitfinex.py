...
class Bitfinex(Feed, BitfinexRestMixin):
    ...
    rest_endpoints = [
        RestEndpoint(
            ...
            routes=Routes(
                ["/v2/conf/pub:list:pair:exchange", "/v2/conf/pub:list:currency", "/v2/conf/pub:list:pair:futures"]
            ),
        )
    ]
    websocket_channels = {
    ...
    async def _book(self, pair: str, msg: list, timestamp: float):
        ...
        if isinstance(msg[1][0], list):
            ...
            if count > 0:
                ...
                self._l2_book[pair].book[side][price] = amount
            # remove price level
            elif price in self._l2_book[pair].book[side]:
    ...
    async def message_handler(self, msg: str, conn: AsyncConnection, timestamp: float):
        ...
        if isinstance(msg, list):
            ...
            if seq_no != expected:
                LOG.warning(
                    "%s: missed message (sequence number) received %d, expected %d", conn.uuid, seq_no, expected
                )
