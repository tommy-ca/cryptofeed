...
class BinanceFutures(Binance, BinanceFuturesRestMixin):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://fstream.binance.com", sandbox="wss://stream.binancefuture.com", options={"compression": None}
        )
    ]
    rest_endpoints = [
        RestEndpoint(
    ...
    def _connect_rest(self):
        ...
        for chan in set(self.subscription):
            if chan == "open_interest":
                addrs = [
                    self.rest_endpoints[0].route("open_interest", sandbox=self.sandbox).format(pair)
                    for pair in self.subscription[chan]
                ]
                ret.append(
                    (
                        HTTPPoll(addrs, self.id, delay=60.0, sleep=self.open_interest_interval, proxy=self.http_proxy),
                        self.subscribe,
    ...
    def _check_update_id(self, pair: str, msg: dict) -> bool:
        if self._l2_book[pair].delta is None and msg["u"] < self.last_update_id[pair]:
            return True
        if msg["U"] <= self.last_update_id[pair] <= msg["u"] or self.last_update_id[pair] == msg["pu"]:
