...
from cryptofeed.defines import ASK, BID, BLOCKCHAIN, BUY, L2_BOOK, L3_BOOK, SELL, TRADES
...
class Blockchain(Feed):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://ws.blockchain.info/mercury-gateway/v1/ws", options={"origin": "https://exchange.blockchain.com"}
        )
    ]
    ...
    async def _pair_l3_update(self, msg: str, timestamp: float):
        ...
        for side in (BID, ASK):
            for update in msg[side + "s"]:
                ...
                elif price in self._l3_book[pair].book[side]:
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan in self.subscription:
            for pair in self.subscription[chan]:
                await conn.write(json.dumps({"action": "subscribe", "symbol": pair, "channel": chan}))
