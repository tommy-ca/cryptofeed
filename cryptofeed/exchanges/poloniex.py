...
from cryptofeed.defines import ASK, BID, BUY, L2_BOOK, POLONIEX, SELL, TRADES
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        if msg["action"] == "snapshot":
            ...
            await self.book_callback(
                L2_BOOK, self._l2_book[pair], timestamp, raw=msg, timestamp=self.timestamp_normalize(data["ts"])
            )
        else:
