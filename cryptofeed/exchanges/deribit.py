...
class Deribit(Feed, DeribitRestMixin):
    id = DERIBIT
    websocket_endpoints = [
        WebsocketEndpoint("wss://www.deribit.com/ws/api/v2", sandbox="wss://test.deribit.com/ws/api/v2")
    ]
    rest_endpoints = [
        RestEndpoint(
    ...
    async def _book_snapshot(self, msg: dict, timestamp: float):
        ...
        self._l2_book[pair].book.bids = {
            Decimal(price): Decimal(amount) for _, price, amount in msg["params"]["data"]["bids"]
        }
        self._l2_book[pair].book.asks = {
            Decimal(price): Decimal(amount) for _, price, amount in msg["params"]["data"]["asks"]
        }
    ...
    async def message_handler(self, msg: str, conn, timestamp: float):
        ...
        if "error" in msg_dict:
            ...
            if "channel" in params:
                ...
                if channel.split(".")[0] == "ticker":
                    await self._ticker(msg_dict, timestamp)

                elif channel.split(".")[0] == "trades":
    ...
    async def _user_channels(self, conn: AsyncConnection, msg: dict, timestamp: float, subchan: str):
        order_status = {"open": OPEN, "filled": FILLED, "rejected": FAILED, "cancelled": CANCELLED, "untriggered": OPEN}
        order_types = {"limit": LIMIT, "market": MARKET, "stop_limit": STOP_LIMIT, "stop_market": STOP_MARKET}
