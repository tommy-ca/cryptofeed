...
class Gemini(Feed, GeminiRestMixin):
    ...
    rest_endpoints = [
        RestEndpoint(
            ...
            routes=Routes("/v1/symbols/details/{}", currencies="/v1/symbols", authentication="/v1/order/events"),
        )
    ]
    request_limit = 1
    ...
    def __reset(self, pairs):
        for pair in pairs:
            self._l2_book[self.exchange_symbol_to_std_symbol(pair)] = OrderBook(
                self.id, self.exchange_symbol_to_std_symbol(pair), max_depth=self.max_depth
            )
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        if self.subscription and (
            (L2_BOOK in self.subscription and msg["symbol"] not in self.subscription[L2_BOOK])
