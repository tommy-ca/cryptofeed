...
from cryptofeed.defines import ASK, BID, BUY, CANDLES, KRAKEN, L2_BOOK, SELL, TICKER, TRADES
...
class Kraken(Feed, KrakenRestMixin):
    ...
    websocket_channels = {L2_BOOK: "book", TRADES: "trade", TICKER: "ticker", CANDLES: "ohlc"}
    request_limit = 10
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan, symbols in conn.subscription.items():
            ...
            await conn.write(json.dumps({"event": "subscribe", "pair": symbols, "subscription": sub}))
    ...
    async def message_handler(self, msg: str, conn, timestamp: float):
        ...
        if isinstance(msg, list):
            ...
            msg["event"] == "heartbeat"
            or msg["event"] == "systemStatus"
            or (msg["event"] == "subscriptionStatus" and msg["status"] == "subscribed")
        ):
            return
        else:
            LOG.warning("%s: Invalid message type %s", self.id, msg)
