...
from cryptofeed.defines import ASK, BID, BITFLYER, BUY, FUTURES, FX, L2_BOOK, SELL, TICKER, TRADES
        ...
        for chan in self.subscription:
            for pair in self.subscription[chan]:
                if chan.startswith("lightning_board"):
                    ...
                    await conn.write(
                        json.dumps({"method": "subscribe", "params": {"channel": f"lightning_board_snapshot_{pair}"}})
                    )
