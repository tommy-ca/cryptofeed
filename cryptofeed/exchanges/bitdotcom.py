...
from cryptofeed.types import Balance, Fill, OrderBook, OrderInfo, Ticker, Trade
...
class BitDotCom(Feed):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://spot-ws.bit.com", instrument_filter=("TYPE", (SPOT,)), sandbox="wss://betaspot-ws.bitexch.dev"
        ),
        WebsocketEndpoint(
            "wss://ws.bit.com",
    ...
    rest_endpoints = [
        ...
        RestEndpoint(
            ...
            routes=Routes(
                "/linear/v1/instruments?currency={}&active=true", currencies=True, authentication="/v1/ws/auth"
            ),
        ),
    ]
    ...
    def get_signature(self, api_path: str, param_map: dict):
        ...
        return hmac.new(
            self.key_secret.encode("utf-8"), str_to_sign.encode("utf-8"), digestmod=hashlib.sha256
        ).hexdigest()
    ...
    async def _book(self, data: dict, timestamp: float):
        ...
        if data["data"]["type"] == "update":
            ...
            self._l2_book[pair] = OrderBook(
                ...
                bids={Decimal(price): Decimal(size) for price, size in data["data"]["bids"]},
                asks={Decimal(price): Decimal(size) for price, size in data["data"]["asks"]},
            )
            self._sequence_no[pair] = data["data"]["sequence"]
            await self.book_callback(
                L2_BOOK,
    ...
    async def _balances(self, msg: dict, timestamp: float):
        ...
        if "balances" in msg["data"]:
            ...
            for balance in msg["data"]["balances"]:
                b = Balance(
                    self.id, balance["currency"], Decimal(balance["available"]), Decimal(balance["frozen"]), raw=msg
                )
