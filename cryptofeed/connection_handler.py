...
class ConnectionHandler:
        ...
        while self._within_retry_budget(retries) and self.running:
            try:
                ...
                await self._handle_retry(
                    exc, delay, LOG.warning, include_exc_message=True
                )
                retries += 1
                delay *= 2
            except Exception as exc:  # pragma: no cover - defensive
                await self._handle_retry(
                    exc, delay, LOG.error, include_exc_message=False
                )
                retries += 1
                delay *= 2
    ...
    async def _handle_retry(
        self, exc: Exception, delay: float, log_method, *, include_exc_message: bool
    ) -> None:
    ...
    async def _handler(self, connection, handler):
        ...
        try:
            ...
            if self.log_on_error:
                # message is guaranteed to be assigned from the async for loop
                log_message = message  # type: ignore[possibly-unbound]
                if connection.uuid in {HUOBI, HUOBI_DM, HUOBI_SWAP}:
                    log_message = zlib.decompress(message, 16 + zlib.MAX_WBITS)  # type: ignore[possibly-unbound]
                elif connection.uuid in {OKCOIN, OKX}:
                    log_message = zlib.decompress(message, -15)  # type: ignore[possibly-unbound]
                LOG.error("%s: error handling message %s", connection.uuid, log_message)
